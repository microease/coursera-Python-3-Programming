...
# sorted_letters = sorted(letters)[::-1]
...
# medals = {'Japan': 41, 'Russia': 56, 'South Korea': 21, 'United States': 121, 'Germany': 42, 'China': 70}
# top_three = []
# for i in sorted(medals.items(), key=lambda item: item[1], reverse=True)[0:3]:
...
# most_needed = []
# for i in sorted(groceries.items(),key=lambda item:item[1],reverse=True):
